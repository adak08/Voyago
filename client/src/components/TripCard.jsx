...
const STATUS_STYLES = {
  upcoming:  "bg-blue-100/90 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300",
  ongoing:   "bg-emerald-100/90 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300",
  completed: "bg-gray-200/80 text-gray-600 dark:bg-gray-800/80 dark:text-gray-400",
};
...
export default function TripCard({ trip, onClick }) {
  const gradient = COVER_GRADIENTS[trip.title.charCodeAt(0) % COVER_GRADIENTS.length];

  return (
    <div onClick={onClick}
      className="card-hover overflow-hidden group">
      {/* Cover image / gradient */}
      <div className={`h-36 bg-gradient-to-br ${gradient} relative flex items-end p-3`}>
        {trip.coverImage && (
        ...
        <span className={`relative badge text-xs backdrop-blur-sm ${STATUS_STYLES[trip.status]}`}>
          {trip.status}
        </span>
      </div>
      ...
      {/* Content */}
      <div className="p-4">
        <h3 className="font-bold text-gray-900 dark:text-gray-100 text-base leading-tight mb-2.5
        ...
        <div className="space-y-1.5 mb-3">
          ...
          <p className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
            <Calendar size={12} className="text-gray-400 dark:text-gray-600 flex-shrink-0" />
