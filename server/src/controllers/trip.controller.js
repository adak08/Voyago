...
export const createTripFromAIPlan = async (req, res, next) => {
    try {
        ...
        const forecastBaseDate = startDate || null;

        const normalizeForecastDate = (entry) => {
            if (entry?.date) {
            ...
            if (
                ...
                dayNumber >= 1 &&
                forecastBaseDate &&
                !Number.isNaN(new Date(forecastBaseDate).getTime())
            ) {
                const d = new Date(forecastBaseDate);
        ...
        const normalizedWeatherForecast = Array.isArray(weather.forecast)
            ? weather.forecast.map((entry) => ({
                  date: normalizeForecastDate(entry),
        ...
        await Itinerary.create({
            ...
            aiData: {
                ...
                budget: {
                    transport:
                        budget?.breakdown?.transport ?? budget?.transport,
                    hotel: budget?.breakdown?.hotel ?? budget?.hotel,
                    food: budget?.breakdown?.food ?? budget?.food,
                    activities:
                        budget?.breakdown?.activities ?? budget?.activities,
            ...
            agentStatus: {
                weather: !!(
                    agentStatus.weather === true || agentStatus.weather === "ok"
                ),
                maps: !!(
                ...
                budget: !!(
                    agentStatus.budget === true || agentStatus.budget === "ok"
                ),
                itinerary: !!(
...
export const leaveTrip = async (req, res, next) => {
    try {
        ...
        try {
            getIO()
                .to(trip._id.toString())
                .emit("member_left", { userId: req.user.id });
        } catch {}
...
export const transferTripAdmin = async (req, res, next) => {
    try {
        ...
        if (!newAdminId) {
            return res
                .status(400)
                .json({ success: false, message: "newAdminId is required" });
        }
        ...
        if (trip.admin.toString() !== req.user.id) {
            return res
                ...
                .json({
                    success: false,
                    message: "Only admin can transfer ownership",
                });
        }
        ...
        if (newAdminId === req.user.id) {
            return res
                .status(400)
                .json({ success: false, message: "You are already the admin" });
        }
        ...
        if (!targetMember) {
            return res
                ...
                .json({
                    success: false,
                    message: "New admin must be a trip member",
                });
        }
    ...
    try {
        ...
        if (trip.admin.toString() !== req.user.id) {
            return res
                ...
                .json({
                    success: false,
                    message: "Only admin can update trip",
                });
        }
    ...
    try {
        ...
        if (trip.admin.toString() !== req.user.id) {
            return res
                ...
                .json({
                    success: false,
                    message: "Only admin can delete trip",
                });
        }
    ...
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
                message:
                    "No file uploaded — check that the form field name is 'photo' and use JPG/PNG/WEBP/GIF/HEIC/HEIF/AVIF",
            });
        }
    ...
    try {
        ...
        if (trip.admin.toString() !== req.user.id) {
            return res
                ...
                .json({
                    success: false,
                    message: "Only admin can send invites",
                });
        }
